...
//! @brief List of graph types available in Designer
enum GraphType
{
	GraphType_Material,
	GraphType_DecalMaterial,
	GraphType_AtlasMaterial,
	GraphType_Filter,
	GraphType_MeshBasedGenerator,
	GraphType_TextureGenerator,
	GraphType_EnvironmentLight,
	GraphType_LightTexture,
	GraphType_UNSPECIFIED,            //!< Empty string
	GraphType_UNKNOWN,
	GraphType_INTERNAL_COUNT
};

//! @brief Helper: get graph type names corresponding to GraphType
//! @return Return array of GraphType_INTERNAL_COUNT strings
const char*const* getGraphTypeNames();
...
struct GraphDesc
{
	...
	//! @brief Graph type as string
	//! Useful if mType is GraphType_UNKNOWN: user specific syntax.
	string mTypeStr;

	//! @brief Per-graph XMP metadata packet in XML encoding
	string mXMP;
	//! @}
	...
	//! @brief Copy of the graph thumbnail as a PNG-encoded image
