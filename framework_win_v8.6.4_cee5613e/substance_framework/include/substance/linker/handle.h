...
/** @brief Get the SBSAR version number of the last pushed assembly
    @param      substanceLinkerHandle Pointer to the Substance handle to query
    @param[out] lastSBSASMVersionNumber Pointer to the version number location
    @return     Returns 0 if succeeded, an error code otherwise. */
SUBSTANCE_EXPORT unsigned int substanceLinkerHandleGetLastSBSASMVersion(
	SubstanceLinkerHandle* substanceLinkerHandle,
	unsigned int*          lastSBSASMVersionNumber);
