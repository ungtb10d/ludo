...
typedef enum
{
	...
	Substance_Linker_Callback_ArchiveExtra, // Only the basename of the file is passed to the callback
	Substance_Linker_Callback_ArchiveExtraFullPath, // The full file path is passed to the callback
	/* etc. */
...
/** @typedef void SubstanceLinkerCallbackArchiveExtra
	...
	This callback can be plugged in the Substance_Linker_Callback_ArchiveExtra
	slot and/or the Substance_Linker_Callback_ArchiveExtraFullPath slot, but only
	one of them will be called. The callback plugged into the
	Substance_Linker_Callback_ArchiveExtraFullPath slot has a higher priority.

	The 'handle' parameter is the Substance handle that calls this callback
	function.<BR>
	The 'filename' parameter contains the name of the extra file inside
	the archive. Depending on whether the callback is registered to the
	Substance_Linker_Callback_ArchiveExtra slot or to the
	Substance_Linker_Callback_ArchiveExtraFullPath slot, it can contain
	either the basename of the extra file (no path), or the complete path
	(minus the common 'assemblies/content/0000' part) of the extra file
	It is a UTF16 null-terminated string. It is no longer valid
	after the callback returns.<BR>
