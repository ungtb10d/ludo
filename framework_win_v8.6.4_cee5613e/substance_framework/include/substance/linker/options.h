...
/** @brief Linking target platform enum
	...
	enum.
	All the x86/x64/SSE values are ignored when targeting an M1 engine */
typedef enum
{
	/** @brief Use current platform
		Target platform is the Architecture/OS of the linker binaries.
		SSE4.1 support is retrieved from current CPU
		The x86/x64/SSE values are ignored when targeting an M1 engine */
